          ...
          <div className="grid grid-cols-3 gap-2">
            {/* "None" / "Default" tile
                - For background, hair, accessory, dress, shoe, decoration, coat:
                  "None" clears the category (selectedItemId === null means None is active)
                - For top & bottom: "None" restores the modesty DEFAULT (since the model
                  is never naked). The tile shows "Default" and is highlighted when the
                  current selection IS the default. */}
            {(() => {
              ...
              // For top/bottom: show the Default tile always (so user can return to default)
              // For others: show None tile always
              return (
                <button
                  type="button"
                  onClick={() => onSelectItem(activeCategory, null)}
